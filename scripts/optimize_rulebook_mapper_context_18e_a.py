"""Run Feature #18E-A context-optimized offline rulebook mapper reports."""
...
from services.pdf_row_context_extraction import build_row_context_report, render_row_context_markdown
...
def _top_records(records: Sequence[Mapping[str, Any]], *, limit: int = 30) -> list[dict[str, Any]]:
    rows = list(records)
...
def _recommend_next(total: int, touched: int, advisory_precision: float | None) -> dict[str, Any]:
    ...
    if advisory_precision is not None and advisory_precision < 0.9:
        next_feature = "Feature #18E-A-hotfix-1 - Tighten context rules and downgrade risky labels before further expansion."
    elif coverage < 0.3:
        next_feature = "Feature #18E-B - Expand mapper using statement-specific template patterns and note-link extraction."
    else:
        next_feature = "Feature #18E-C - Compare context-optimized mapper against Qwen on the same offline benchmark."
    return {
...
def _render_optimized_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Rulebook Mapper Context Optimized - Feature #18E-A",
        "",
        ...
        f"- Context optimizations applied: {summary.get('context_optimization_applied_count', 0)}",
...
def _render_comparison_markdown(report: Mapping[str, Any]) -> str:
    summary = report.get("summary") or {}
    lines = [
        "# Rulebook Mapper Context Coverage Comparison - Feature #18E-A",
        "",
        "| Metric | Baseline #18D-C | Context optimized #18E-A |",
        "| --- | ---: | ---: |",
    ]
    for key in (
        ...
        lines.append(
            f"| {key} | {summary.get('baseline', {}).get(key)} | {summary.get('context_optimized', {}).get(key)} |"
        )
...
def _render_no_match_markdown(report: Mapping[str, Any]) -> str:
    summary = report.get("summary") or {}
    lines = [
        "# Rulebook Mapper Context No-Match Rows - Feature #18E-A",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimize #18D-C rulebook mapper suggestions with cached row context.")
    ...
    parser.add_argument("--output-dir", default="reports", help="Directory for #18E-A reports.")
...
def main() -> int:
    ...
    baseline_evaluated = _evaluate_records(baseline_records, row_values=row_values, facts_by_sample=facts_by_sample)
    optimized_evaluated = _evaluate_records(optimized_records, row_values=row_values, facts_by_sample=facts_by_sample)

    optimized_summary = summarize_mapper_records(optimized_records, samples=samples, rules=rules, generated_at=generated_at)
    optimized_summary["feature"] = "18E-A"
    optimized_summary["context_optimization_applied_count"] = sum(1 for item in optimized_records if item.get("context_optimization_applied"))
    optimized_summary["local_xbrl_evaluation"] = _eval_summary(optimized_evaluated)
    optimized_summary["explicit_auto_apply_statement"] = "No #18E-A suggestion is safe for auto-apply; human review remains required."

    baseline_summary = dict(baseline_reports["summary"]["summary"])
    baseline_eval = _eval_summary(baseline_evaluated)
    baseline_touched = int(baseline_summary.get("advisory_suggestions_count") or 0) + int(baseline_summary.get("review_required_suggestions_count") or 0) + int(baseline_summary.get("conflicts_count") or 0)
    optimized_touched = int(optimized_summary.get("advisory_suggestions_count") or 0) + int(optimized_summary.get("review_required_suggestions_count") or 0) + int(optimized_summary.get("conflicts_count") or 0)
    total = int(optimized_summary.get("total_pdf_row_value_observations") or 0)
    recommendation = _recommend_next(total, optimized_touched, optimized_summary["local_xbrl_evaluation"].get("advisory_precision_on_evaluable"))
    ...
    optimized_report = {
        ...
        "suggestions": optimized_evaluated,
    }
    comparison_report = {
        "run_metadata": run_metadata,
        "summary": {
            "baseline": {
                ...
                "touched_coverage_rate": _safe_rate(baseline_touched, int(baseline_summary.get("total_pdf_row_value_observations") or 0)),
                "advisory_precision_on_evaluable": baseline_eval.get("advisory_precision_on_evaluable"),
            },
            "context_optimized": {
                ...
                "advisory_precision_on_evaluable": optimized_summary["local_xbrl_evaluation"].get("advisory_precision_on_evaluable"),
            },
            "improvement": {
    ...
    errors = [
        item
        for item in optimized_evaluated
        if item.get("predicted_qname") and item.get("evaluation_status") in (FALSE_POSITIVE_STATUSES | NOT_EVALUABLE_STATUSES)
    ]
    errors_report = {
        "run_metadata": run_metadata,
        "summary": {
            "false_positive_count": sum(1 for item in optimized_evaluated if item.get("evaluation_status") in FALSE_POSITIVE_STATUSES),
            "not_evaluable_count": sum(1 for item in optimized_evaluated if item.get("predicted_qname") and item.get("evaluation_status") in NOT_EVALUABLE_STATUSES),
            "evaluation_status_counts": optimized_summary["local_xbrl_evaluation"].get("evaluation_status_counts"),
            "safety": SAFETY,
        },
        "errors": _top_records(errors, limit=80),
    }
    no_match_rows = _no_match(optimized_records)
    no_match_report = {
        "run_metadata": run_metadata,
        "summary": {
            "no_match_count": len(no_match_rows),
            "top_no_match_labels": _top_no_match_labels(no_match_rows, limit=40),
            "safety": SAFETY,
        },
    ...
    paths = {
        "row_context_json": output / "pdf_row_context_extraction_18e_a.json",
        "row_context_md": output / "pdf_row_context_extraction_18e_a.md",
        "optimized_json": output / "rulebook_mapper_context_optimized_18e_a.json",
        "optimized_md": output / "rulebook_mapper_context_optimized_18e_a.md",
        "comparison_json": output / "rulebook_mapper_context_coverage_comparison_18e_a.json",
        "comparison_md": output / "rulebook_mapper_context_coverage_comparison_18e_a.md",
        "errors_json": output / "rulebook_mapper_context_errors_18e_a.json",
        "errors_md": output / "rulebook_mapper_context_errors_18e_a.md",
        "no_match_json": output / "rulebook_mapper_context_no_match_18e_a.json",
        "no_match_md": output / "rulebook_mapper_context_no_match_18e_a.md",
    }
    _write_json(paths["row_context_json"], row_context_report)
    paths["row_context_md"].write_text(render_row_context_markdown(row_context_report), encoding="utf-8")
    ...
    print("Feature #18E-A context-optimized mapper reports written:")
    for name, path in paths.items():
        print(f"- {name}: {path}")
    print("")
    print("Coverage comparison:")
    print(f"- baseline_touched_coverage_rate: {comparison_report['summary']['baseline']['touched_coverage_rate']}")
    print(f"- optimized_touched_coverage_rate: {comparison_report['summary']['context_optimized']['touched_coverage_rate']}")
    print(f"- optimized_advisory_precision_on_evaluable: {comparison_report['summary']['context_optimized']['advisory_precision_on_evaluable']}")

...
DEFAULT_PLAN_REPORT_TEMPLATE = "reports/receivables_persisted_row_correction_plan_{job_id}.json"
DEFAULT_DRY_RUN_REPORT_TEMPLATE = (
    "reports/receivables_template_field_correction_11p_dry_run_{job_id}.json"
)
DEFAULT_APPLY_REPORT_TEMPLATE = (
    "reports/receivables_template_field_correction_11p_apply_{job_id}.json"
)

RECEIVABLES_TEMPLATE_FIELD_IDS = (
...
def load_target_ids_from_plan(
    ...
    if report.get("feature") != "11O":
        raise ValueError("Target report must be the Feature #11O correction plan.")
    ...
    if report.get("apply_supported") is not False:
        raise ValueError("Target report must be the dry-run-only #11O plan.")
    ...
    for row in candidate_rows:
        ...
        if not is_trade_and_other_current_receivables_concept(
            row.get("current_template_field_id")
        ):
            raise ValueError("Every target row must have a TradeAndOtherCurrentReceivables template_field_id.")
        label = row.get("extracted_label")
        if automatic_mapping_guardrail_reason(
            row.get("current_template_field_id"),
            label,
        ) != "receivables_detail_guardrail":
            raise ValueError("Every target row must still be blocked by the receivables guardrail.")
        if not label_looks_like_receivables_detail_row(label):
            raise ValueError("Every target row must be a company/customer-like receivables detail label.")
...
def build_guardrail_failure_predicate():
    ...
    evidence_present = or_(*[label.like(f"%{term}%") for term in RECEIVABLES_EVIDENCE_TERMS])
    detail_present = or_(*[label.like(f"%{term}%") for term in RECEIVABLES_DETAIL_LABEL_TERMS])
    return and_(detail_present, not_(evidence_present))
...
def build_correction_update_statement(
    ...
    return (
        ...
        .where(ExtractedDataItem.template_field_id.in_(RECEIVABLES_TEMPLATE_FIELD_IDS))
...
def _eligibility_reason(row: Any, expected_job_id: int) -> tuple[bool, str]:
    ...
    if not is_trade_and_other_current_receivables_concept(concept_id):
        return False, "not eligible: current template_field_id is no longer TradeAndOtherCurrentReceivables"
    ...
    if automatic_mapping_guardrail_reason(concept_id, label) != "receivables_detail_guardrail":
        return False, "not eligible: current extracted_label now supports receivables/debtor/due-from mapping"

    if not label_looks_like_receivables_detail_row(label):
        return False, "not eligible: current extracted_label no longer looks like a company/customer detail row"

    return (
        True,
        "eligible: exact target row still has TradeAndOtherCurrentReceivables template_field_id and company/customer-like detail label without explicit receivable evidence",
    )
...
def build_report(
    ...
    return {
        ...
        "update_policy": {
            ...
            "requires_selected_job_id": True,
            "requires_current_trade_and_other_current_receivables_template_field_id": True,
            "requires_company_customer_detail_label_without_explicit_receivable_evidence": True,
...
def render_report(report: Dict[str, Any]) -> str:
    lines = [
        "Feature #11P receivables template_field_id correction",
        f"Mode: {report['mode']}",
        "",
    ...
    for row in report["target_rows"]:
        lines.append(
            ...
            "proposed_action=clear template_field_id only; no replacement concept; no aggregation/dimensions "
    ...
    lines.extend(
        [
            ...
            "Note: apply mode clears only template_field_id for exact target rows still matching the receivables guardrail predicate.",
...
async def async_main() -> int:
    parser = argparse.ArgumentParser(
        description="Correct exact Feature #11P receivables template_field_id rows."
    )
    parser.add_argument("--job-id", type=int, default=DEFAULT_JOB_ID, help="Filing job ID to correct.")
    parser.add_argument(
        "--plan-report",
        default=None,
        help="Feature #11O plan report to load target IDs from.",
    )
    parser.add_argument(
        "--report-path",
        default=None,
